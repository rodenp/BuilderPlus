...
export const getHTML = (component: CanvasComponent): string => {
  ...
  const label = (props.label as string) || 'Checkbox label';

  return `<div style="${wrapperStyle}"><input type="checkbox"${checked ? ' checked' : ''} style="width: 16px; height: 16px" /><label style="${labelStyle}">${label}</label></div>`;
};