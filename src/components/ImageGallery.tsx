...
const useStore = () => {
    // This is a placeholder. In a real port, we'd hook this up to the builder's state or a media manager
    const [gallery, setGallery] = useState<GalleryImage[]>([]);

    return {
        imageGallery: gallery,
        addImageToGallery: (img: Partial<GalleryImage>) => {
            const newImg = { ...img, id: Math.random().toString(), createdAt: new Date().toISOString() } as GalleryImage;
            setGallery(prev => [...prev, newImg]);
            return newImg;
        },
        removeImageFromGallery: (id: string) => setGallery(prev => prev.filter(i => i.id !== id)),
        updateGalleryImage: (id: string, updates: Partial<GalleryImage>) => setGallery(prev => prev.map(i => i.id === id ? { ...i, ...updates } : i))
    };
};
